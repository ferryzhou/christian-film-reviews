...
依赖：`pip install python-docx ebooklib opencc-python-reimplemented pillow weasyprint pymupdf`，并安装 Noto Serif CJK SC/TC 字体。封面与印刷封面共用一套设计（`cover_designs.py` 里的几套方案，`book.json` 的 `cover.design` 选用，当前为 `door` 门缝透光），由 WeasyPrint 渲染，不依赖 Chromium。
